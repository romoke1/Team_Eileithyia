...
class Model_getvalues extends CI_Model {
    ...
    function login_teacher($email, $pass) {
    ...
    function login_student($email, $pass) {
        $this->db->where('email', $email);
        $this->db->where('password', $pass);
    ...
    function getTableRows2($table, $where, $whereVal, $where2, $whereVal2, $orderby, $orderVal = "desc", $limit = 0) {
        $this->db->select('*');
        $this->db->where($where, $whereVal);
    ...
    function searchValLikeArr($table, $array, $orderby, $orderVal = "desc") {
        $this->db->select('*');
        foreach ($array as $val) {
            $this->db->like('search_all', $val);
        }
        $this->db->order_by($orderby, $orderVal);
        $this->db->from($table);
        $query = $this->db->get();
    ...
    function searchValLikeArr2($table, $whereArray, $array, $whereNew, $newVal, $orderby, $orderVal = "desc") {
        $this->db->select('*');
        $this->db->where($whereArray);
        foreach ($array as $val) {
            $this->db->like('search_all', $val);
        }
        $this->db->where($whereNew, $newVal);
        $this->db->order_by($orderby, $orderVal);
        $this->db->from($table);
        $query = $this->db->get();
        return $query->result();
    ...
    function searchValWhere($table, $where, $whereVal, $where2, $whereVal2, $orderby, $orderVal = "desc") {
        $this->db->select('*');
        //$this->db->where();
        $this->db->where($where2, $whereVal2);
        $this->db->like($where, $whereVal);
    ...
    function searchVal2($table, $array, $orderby, $orderVal = "desc") {
        $this->db->select('*');
        $this->db->where($array);
        //$this->db->where('amount BETWEEN 1500000 AND 2000000');
        $this->db->order_by($orderby, $orderVal);
        $this->db->from($table);
        $query = $this->db->get();
        return $query->result();
