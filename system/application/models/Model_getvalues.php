...
class Model_getvalues extends CI_Model {
    ...
    //---- For Dealer ----------
     ...
     function login_dealers($email, $pass) {
    ...
    function verify_code($dealer_id) {
        $this->db->where('verify_code', $this->input->post('txtVerify'));
    ...
    //-- End of Dealer Model function

     function login_ad($email, $pass) {
        $this->db->where('email', $email);
    ...
    function searchValLikeArr2($table, $whereArray, $array, $orderby, $orderVal = "desc") {
        $this->db->select('*');
